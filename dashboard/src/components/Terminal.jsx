...
export const Terminal = ({ logs, onResize, isResizing, nodeRegistry = {} }) => {
  ...
  return (
    <div className={`flex flex-col h-full bg-slate-900 overflow-hidden border-t border-slate-800 shadow-2xl relative ${isResizing ? 'select-none' : ''}`}>
      ...
      <div
        className="absolute top-0 left-0 right-0 z-50 cursor-ns-resize group"
        style={{ height: 6 }}
        onMouseDown={onResize}
      >
        <div style={{
          width: '100%', height: 2, transition: 'background 0.2s',
      ...
      <div className="px-5 py-3 bg-slate-900/80 border-b border-slate-800 flex flex-col gap-2 select-none">
        <div className="flex items-center gap-6 text-[9px] font-bold uppercase tracking-widest text-slate-500 pb-1 border-b border-slate-800/50">
          <span style={{ width: '120px', flexShrink: 0 }}>Node_ID</span>
          <span style={{ width: '80px', flexShrink: 0 }}>Status</span>
          <span className="flex-1">Last_Model_Hash</span>
          <span style={{ width: '48px', textAlign: 'right', flexShrink: 0 }}>Rep</span>
        </div>
        <div className="flex flex-col gap-1 max-h-24 overflow-y-auto">
          {Object.entries(nodeRegistry).length === 0 ? (
            ...
            Object.entries(nodeRegistry).map(([id, info]) => (
              <div key={id} className="flex items-center gap-6 text-[10px] font-mono tracking-tight text-slate-300">
                <span style={{ width: '120px', flexShrink: 0 }} className="font-bold text-indigo-400">{id}</span>
                <span style={{ width: '80px', flexShrink: 0 }} className="flex items-center gap-2">
                ...
                <span style={{ width: '48px', textAlign: 'right', flexShrink: 0 }} className="text-emerald-500 opacity-60">1.0</span>
              </div>
            ))
          )}
        </div>
